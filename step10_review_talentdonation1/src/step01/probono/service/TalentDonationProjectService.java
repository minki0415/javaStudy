...
package step01.probono.service;
...
public class TalentDonationProjectService {
	...
	public void donationProjectInsert(TalentDonationProject project) {
		donationProjectList[index] = project;
		index++; //index = index + 1
	}
	...
	public void donationProjectDelete(String projectName) {
		...
		for(int i=0; i< donationProjectList.length ; i++) {
			project = donationProjectList[i];
			
			if(project != null && project.getTalentDonationProjectName().equals(projectName)) {
				donationProjectList[i] = null;//참조했던 객체를 쓰레기로 만들고 null로 대입해서 배열에서 삭제
				break;
			}
